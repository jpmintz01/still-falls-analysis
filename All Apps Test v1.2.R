...
pw_all_data$other.decision <- NA
for (j in pw_ids){
for (i in 1:10) {
    pw_all_data[pw_all_data$period==i & pw_all_data$Adversary=="Human" & pw_all_data$id==j,]$other.decision <- human_adv_choices[i]
    pw_all_data[pw_all_data$period==i & pw_all_data$Adversary=="AI"& pw_all_data$id==j,]$other.decision <- ai_adv_choices[i]
    pw_all_data[pw_all_data$period==i & pw_all_data$Adversary=="Human+AI"& pw_all_data$id==j,]$other.decision <- hai_adv_choices[i]
}
}
...
for (i in rps_ids) {
  ...
  for (j in Adversary_list) {
    ...
    b.zoo[b.zoo$id==i & b.zoo$Adversary==j,"Choice.1"] <- lag(b.zoo[b.zoo$id==i & b.zoo$Adversary==j,"Choice_of_Advisor"], 1)
    b.zoo[b.zoo$id==i & b.zoo$Adversary==j,"Adversary.1"] <- j
    b.zoo[b.zoo$id==i & b.zoo$Adversary==j,"Outcome.1"] <- lag(b.zoo[b.zoo$id==i & b.zoo$Adversary==j,"Payoff"], 1)
  }
}
b<-as.data.frame(b.zoo)
rps_all_data_with_demo <-b
...
print(paste0("SD is ",if(l$sd<l$mean){"NOT "},"greater than the mean, so data is ",if(l$sd<l$mean){"NOT "},"overdispersed."))
...
print(xtabs(~Adversary+my.decision, data=pw_all_data_with_demo[pw_all_data_with_demo$period==1,]))
print("")
...
# a<-ad_p10
# a$other.d <- as.factor(a$other.d)
# p<-ggplot(a, aes(x=my.d, group=other.d, y=..density..))+
#   geom_histogram(aes(fill=other.d), position="dodge")+
...
a<- xtabs(~Adversary+pw_order+my.decision, data=pw_all_data_with_demo[pw_all_data_with_demo$period==1,])
print(a)
n<-a[,,1]/(a[,,1]+a[,,2]) #calculate cooperation ratios
print(n)
print(wilcox.test(n[,1], n[,2]))

# m <- glmer(my.decision ~ pw_order + (1|id), data=pw_all_data_with_demo[pw_all_data_with_demo$period==1,], family=binomial(link="logit"))
# print(Anova(m))
print("RESULT:  a mann-whitney U test on ROUND ONE decisions and pw_order DID NOT show a statistically significant difference in cooperativness by competitor at p <.05., but it was a finding of statistical interest (p<0.10)")
...
a<- xtabs(~Adversary+pw_order+my.decision, data=pw_all_data_with_demo)
print(a)
n<-a[,,1]/(a[,,1]+a[,,2]) #calculate cooperation ratios
print(n)
print(wilcox.test(n[,1], n[,2]))
# print(chisq.test(xtabs(~Adversary+ pw_order + my.decision, data = pw_all_data_with_demo)[,,1]))
print("Mann-Whitney U test of cooperativeness by competior and play order showed no significant difference in variation at p<0.05")
a<- xtabs(~pw_order+my.decision, data=pw_all_data_with_demo)
print(a)
chisq.test(a)
print("An interesting finding unrelated to the hypotheses is that game order did show a significant relationship with counterbalancing group cooperativeness (X-squared = 12.87, df = 1, p-value = 0.0003339) with those who played the IPD second being much less cooperative than those who played IPD first. This is worth of future research.")
...
print(xtabs(~ gender + my.decision, data = pw_all_data_with_demo))
print(chisq.test(xtabs(~ gender + my.decision, data = pw_all_data_with_demo)))
print(chisq.test(xtabs(~ Adversary + gender + my.decision, data = pw_all_data_with_demo)[,,2]))
...
print(chisq.test(xtabs(~ Adversary + service + my.decision, data = pw_all_data_with_demo)[,,2]))
...
print(chisq.test(xtabs(~ Adversary + rank + my.decision, data = pw_all_data_with_demo)[,,2]))
print("Chi-sq (aggregate) on whether RANK affected decision-making was NOT statistically significant in general or by competitor at p < .05 ")


print(l<-chisq.test(xtabs(~ age + my.decision, data = pw_all_data_with_demo)))
print(l<-chisq.test(xtabs(~ Adversary+age + my.decision, data = pw_all_data_with_demo)[,,2]))
...
print(l<-chisq.test(xtabs(~ Adversary+years_military_experience+ my.decision, data = pw_all_data_with_demo)[,,2]))
...
print(chisq.test(xtabs(~ Adversary + school + my.decision, data = pw_all_data_with_demo)[,,2]))
print("RESULT: Chi-sq (aggregate) shows cooperativeness is related to school, but not to variation by adversary at p < .05 ")

print(xtabs(~  school + my.decision, data = pw_all_data_with_demo)[,2]/(rowSums(xtabs(~  school + my.decision, data = pw_all_data_with_demo))))
...
print(chisq.test(xtabs(~Adversary+game_theory_experience + my.decision, data=pw_all_data_with_demo)[,,2]))
...
print(chisq.test(xtabs(~ Adversary + machine_learning_experience + my.decision, data = pw_all_data_with_demo)[,,2])) #not significant
...
#reponse variable is match frequency of advisor choice
#factor (IV) is adversary
a<-as.data.frame(xtabs(~id+Choice_of_Advisor+Adversary, data=rps_all_data_with_demo))
print("AI Delegation characterization")
l<-descdist(a[a$Choice_of_Advisor=="AI",]$Freq, discrete = FALSE) #2nd row = coop
print(l)
print("how to insert/save cullen & freq graph of data")
print("RESULT: Cullen & Frey plot indicates a abnormal distributon for aggregate data.")
print(paste("Mean AI ADvisor Choice:", l$mean))
getmode <- function(v) {
  uniqv <- unique(v)
  uniqv[which.max(tabulate(match(v, uniqv)))]
}
print(paste("Mode AI ADvisor Choice:", getmode(a[a$Choice_of_Advisor=="AI",]$Freq)))
print(paste("Median AI ADvisor Choice:", l$median))
print(paste("Range of AI ADvisor Choice:", paste(l$min,l$max, collapse="-")))
print(paste("Standard Dev:",l$sd))
print(paste("Variance:",l$sd^2))
print(paste0("SD is ",if(l$sd<l$mean){"NOT "},"greater than the mean, so data is ",if(l$sd<l$mean){"NOT "},"overdispersed."))


print("No Delegation ('none') characterization")
l<-descdist(a[a$Choice_of_Advisor=="none",]$Freq, discrete = FALSE) #2nd row = coop
print(l)
print("how to insert/save cullen & freq graph of data")
print("RESULT: Cullen & Frey plot indicates a abnormal distributon for aggregate data.")
print(paste("Mean AI ADvisor Choice:", l$mean))
print(paste("Mode AI ADvisor Choice:", getmode(a[a$Choice_of_Advisor=="none",]$Freq)))
print(paste("Median AI ADvisor Choice:", l$median))
print(paste("Range of AI ADvisor Choice:", paste(l$min,l$max, collapse="-")))
print(paste("Standard Dev:",l$sd))
print(paste("Variance:",l$sd^2))
...
print("Human Delegation characterization")
l<-descdist(a[a$Choice_of_Advisor=="human",]$Freq, discrete = FALSE) #2nd row = coop
print(l)
print("how to insert/save cullen & freq graph of data")
print("RESULT: Cullen & Frey plot indicates a abnormal distributon for aggregate data.")
print(paste("Mean AI ADvisor Choice:", l$mean))
print(paste("Mode AI ADvisor Choice:", getmode(a[a$Choice_of_Advisor=="human",]$Freq)))
print(paste("Median AI ADvisor Choice:", l$median))
print(paste("Range of AI ADvisor Choice:", paste(l$min,l$max, collapse="-")))
print(paste("Standard Dev:",l$sd))
print(paste("Variance:",l$sd^2))
...
p<- ggplot(data=rps_df_1) + 
  geom_bar(aes(x = Freq,fill=Choice_of_Advisor)) + 
  facet_wrap(~Choice_of_Advisor)+
  labs(title="RPS - Delegation Histograms", x="Frequency", y = "Count of Choices", color = "Choice_of_Advisor") +
  scale_x_discrete(limits=c(0:10))+
...
# test 1 - large size (nope)
print("Test for large sample size (>20 in each group):")
print(paste("largest counterbalancing group size:",max(xtabs(~rps_order, data=rps_all_data_with_demo)/30)))
#test 2 - normal distribution (nope)
...
print("RESULT: All three response variables show significant departures from normality")
#test 3 - significant outliers (noper)
a<-as.data.frame(xtabs(~id+Choice_of_Advisor+Adversary, data=rps_all_data_with_demo))
...
print(paste0("Insert ", p$labels$title," Plot"))
ggsave(paste0(p$labels$title,".pdf"), plot=p, device="pdf")
print("RESULT: there are significant outliers in the human condition")

a<-as.data.frame(xtabs(~id+Choice_of_Advisor+Adversary, data=rps_all_data_with_demo))

p<- ggplot(a, aes(x=Freq)) + geom_bar(aes(y = ..prop..,group=Choice_of_Advisor, fill=Choice_of_Advisor),position="dodge", color="black")+
  geom_density(aes(color=Choice_of_Advisor),alpha=0.7, show.legend = F)+
  labs(x = "Frequency", y = "Density", fill="Delegate",
       title = "RPS - Delegate Distributions")+
...
# names(d)[2] <- "topChoice"
# d[,3] <- NULL
# print(paste0("Amplifying: ",length(which(d$topChoice=="none"))," participants chose 'no advice' as their top choice, and the remaining ",length(rps_ids)-length(which(d$topChoice=="none"))," chose to delegate as their top choice."))
...
print("RESULT: 18 (75%) of participants chose 'no advice' as their top choice, and the remaining 6 (25%) chose the AI")
...
p<- ggplot(data=df_1, aes(x=Round))+
  ...
  geom_line(aes(y=Freq,group=Choice_of_Advisor, color=Choice_of_Advisor))+ 
  labs(title="RPS-Choices by Round and Adversary",x="Round", y = "# of Choices", color = "Advisor Choice")+
  geom_smooth(method='glm',formula=y~x,aes(x=Round,y=Freq,group=Choice_of_Advisor, color=Choice_of_Advisor), se=FALSE)
print(p)
print("make this a bar chart if possible")
print(paste0("Insert ", p$labels$title," Plot"))
ggsave(paste0(p$labels$title,".jpg"), plot=p, device="jpg")
print("RESULT (further research): See PLOT: It appears the type of adversary may have mediated variation in choice of advisor over time - vs the AI, use of the AI advisor dropped (while it increased in other treatments). Vs the human, use of the human advisor went up less than it did in the other treatments. Possible greater decrease in 'no-delegation'(self-reliance) in the vs Human+AI treatment compared to the other treatments.")


print("HELP? Should it be this: print(mantelhaen.test(xtabs(~Adversary+Choice_of_Advisor+Round, data=rps_all_data_with_demo))) or this:")
...
print("COUNTERBALANCING CHECK for delegation propensity hypothesis")
#participant's propensity to delegate (by order)
print(n<-xtabs(~  pw_order+Choice_of_Advisor , data = rps_all_data_with_demo))
print(n<-cbind(n,n[,1]+n[,2]))
print(n[,c(3,4)])
print(x<-chisq.test(n[,c(3,4)]))
print(cramerV(n[,c(3,4)]))

print("result: game order showed a significant (p=0.001) but small(CramerV = .12 (V~.10=small) df=1) relationship with propensity to delegate - order effects")
# print(n<-as.data.frame(xtabs(~  Choice_of_Advisor+rps_order+id , data = rps_all_data_with_demo)))
# 
# for (i in rps_ids){
#   for (j in levels(n$rps_order)) {
#   
#   if (sum(n[n$id==i & n$rps_order==j,]$Freq)==0){
#     n[n$rps_order==j & n$id==i,]$Freq <- NA
#   }
#   }
# }
...
print(n<-cbind(n,n[,1]+n[,2]))
print(x<-chisq.test(n[,c(3,4)]))
# print(x<-wilcox.test(n[,3],n[,4]))
print(CramerV(n[,c(3,4)]))
print("result: RPS Adversary did showed a significant (p<2.2e-16) and moderate (Cramer V = .366 df(cramer) = 1) relationship with propensity to delegate - order effects")

#participants who delegate's propensity for AI advisor (by order)
print(n<-xtabs(~  pw_order+Choice_of_Advisor , data = rps_all_data_with_demo))
print(n[,-3])
print(x<-chisq.test(n[,-3]))
print(CramerV(n[,-3]))
print("result: of those that delegated, game order showed a significant (p=.0007) but small-moderate (CramerV V=.22) relationship with propensity to delegate to AI vs human - order effects")

print(n<-xtabs(~  rps_order+Choice_of_Advisor , data = rps_all_data_with_demo))
print(n[,-3])
print(x<-wilcox.test(n[,1],n[,2]))
print(x<-chisq.test(n[,-3]))
print(CramerV(n[,-3]))
print("result: of those that delegated, RPS Adversary showed a significant (p.002674) but moderate (CramerV = .2612 CramerVdf = 1) relationship with propensity to delegate to AI vs human - order effects")

# print("Given the (chisq/wilcox) relationship between game order and delegation propensities, a friedman test will determine whether there is a directional difference")
# print(n<-xtabs(~  pw_order+Choice_of_Advisor +id, data = rps_all_data_with_demo))
# print(x<-friedman.test(t(n)))
# print("result - friedman tests show no directionality as a result of pw_order")
# 
# print(n<-xtabs(~  rps_order+Choice_of_Advisor , data = rps_all_data_with_demo))
# print(friedman.test(t(n)))
# print("result - friedman tests show no directionality as a result of rps_order")

print("COUNTERBALANCING CHECK for Adversary-Choice hypothesis")
print(mantelhaen.test(xtabs(~  Choice_of_Advisor +Adversary+pw_order, data = rps_all_data_with_demo)))
print("RESULT: A CMH/Mantelhaen test shows that choice of advisor and adversary are still statistically unrelated even after accounting for pw_order at p < .05")
print(mantelhaen.test(xtabs(~ Choice_of_Advisor +Adversary+rps_order, data = rps_all_data_with_demo)))
print("RESULT: A CMH/Mantelhaen test shows that choice of advisor and adversary are still statistically unrelated even after accounting for rps_order at p < .05")
...
print(chisq.test(n))
print(CramerV(n))
print("Chi-sq test on aggregate shows gender was correlated to Choice_of_Advisor, p < .05 but CramerV is small")
...
print("CMH test on aggregate shows GENDER did not affect decision-making BY ADVERASRY, p < .05")
# print("IS THIS CORRECT WAY OF DOING THIS?")
# print(n<-xtabs(~Choice_of_Advisor+age , data = rps_all_data_with_demo))
# print(l<-friedman.test(n))
# print(n<-as.data.frame(xtabs(~  age+Choice_of_Advisor, data = rps_all_data_with_demo)))
# friedman.test(xtabs(~  age+Choice_of_Advisor, data = rps_all_data_with_demo))
chisq.test(xtabs(~  age+Choice_of_Advisor, data = rps_all_data_with_demo))
# print(friedman.test(Freq~Choice_of_Advisor|age, data=n))
# print(KendallW(n, test=TRUE))
print("Friedman test on aggregate shows YEARS OF MILITARY EXPERIENCE did affect decision-making in general, p < .01 with a ")
...
# print(n<-xtabs(~years_military_experience+topChoice , data = rps_all_data_with_demo))
# print(l<-friedman.test(t(n)))
# print("Friedman test on top Choice shows self-reported years_military_experience was not correlated to choice of advisor at p < .05")

print(n<-as.data.frame(xtabs(~  years_military_experience+Choice_of_Advisor, data = rps_all_data_with_demo)))
print(friedman.test(Freq~Choice_of_Advisor|years_military_experience, data=n))
# print(KendallW(n, test=TRUE))
print("Friedman test on aggregate shows YEARS OF MILITARY EXPERIENCE did affect decision-making in general, p < .01 with a ")
...
# print("IS THIS CORRECT WAY OF DOING THIS?")
print(n<-xtabs(~  Choice_of_Advisor+rank , data = rps_all_data_with_demo))
print(chisq.test(n))
print(CramerV(n))
print("Chi-sq test on aggregate shows RANK did affect decision-making in general, p < .05 but the effect size was small")
...
print(n<-xtabs(~  Choice_of_Advisor+school , data = rps_all_data_with_demo))
print(chisq.test(n))
print(CramerV(n))
print("Chi-sq test on aggregate shows SCHOOL did affect decision-making in general, p < .01 but efefct size was small")
...
print(chisq.test(n))
print(CramerV(n))
print("Chi-sq test on aggregate shows SERVICE did affect decision-making in general, p < .01. but effect size was small")
...
a <- rownames(a[a$`1` %in% c(0,30,1,29),])
